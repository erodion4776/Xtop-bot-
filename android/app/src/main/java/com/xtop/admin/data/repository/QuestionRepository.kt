...
class QuestionRepository {
    ...
    suspend fun getQuestions(courseId: String): List<CourseQuestion> {
        return db.from("course_questions").select {
            filter { eq("course_id", courseId) }
        }.decodeList<CourseQuestion>()
    }

    suspend fun createQuestion(question: CourseQuestion): CourseQuestion {
        return db.from("course_questions").insert(question) { select() }.decodeSingle<CourseQuestion>()
    }
    ...
    suspend fun getExamConfig(courseId: String): ExamConfig? {
        return try {
            db.from("course_exam_config").select {
                filter { eq("course_id", courseId) }
            }.decodeSingleOrNull<ExamConfig>()
        } catch (e: Exception) {
            null
        }
    }
