...
// ═══════════════════════════════════════════════════════
// LEADS / QUOTATIONS / AGENT REQUESTS (Phase 3)
// ═══════════════════════════════════════════════════════
...
export async function getStudentCourseAccess(
  ...
  const { data: existing, error } = await sb.from("student_courses").select("*")
  ...
  const { data: created, error: ce } = await sb.from("student_courses").insert({
...
export async function getCourseLessons(courseId: string): Promise<CourseLesson[]> {
  ...
  const { data, error } = await sb.from("course_lessons").select("*")
    .eq("course_id", courseId).eq("status", "ACTIVE").order("lesson_order", { ascending: true });
...
export async function markLessonComplete(
  ...
  const sb = getSupabaseClient();
  const { data: access, error: fetchErr } = await sb.from("student_courses")
  ...
  const { error } = await sb.from("student_courses")
